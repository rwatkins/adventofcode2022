...
monkeyInspectItem :: Monkey -> Item -> Throw
monkeyInspectItem monkey@Monkey {operation, testDivisibleBy, throwTo} item =
  let it = applyOperation operation item
      itemDiv3 = it `div` 3
      throwToMonkey =
        if divisibleBy testDivisibleBy itemDiv3
          then fst throwTo
          else snd throwTo
   in Throw {item = itemDiv3, monkey = throwToMonkey}

monkeyInspectAll :: Monkey -> (Monkey, [Throw])
monkeyInspectAll monkey =
  let throws = L.foldl' f [] monkey.items
   in (monkey {items = []}, throws)
  where
    f throws item = (monkeyInspectItem monkey item) : throws
...
doRound :: World -> World
doRound (monkeys, pendingThrows, inspectCounts) =
  ...
  where
    ...
    f (seenMonkeys, pendingThrows, counts) (monkeyIdx, monkey) =
      let (mpending, pending) = mapSwap monkeyIdx [] pendingThrows
          -- Inspect all items
          (newMonkey, throws) = monkeyInspectAll monkey {items = monkey.items ++ (fromMaybe [] mpending)}
          -- Record throws
...
testInput :: [Monkey]
testInput =
  [ Monkey {items = [79, 98], operation = Mult "old" "19", testDivisibleBy = 23, throwTo = (2, 3)},
    Monkey {items = [54, 65, 75, 74], operation = Add "old" "6", testDivisibleBy = 19, throwTo = (2, 0)},
    Monkey {items = [79, 60, 97], operation = Mult "old" "old", testDivisibleBy = 13, throwTo = (1, 3)},
    Monkey {items = [74], operation = Add "old" "3", testDivisibleBy = 17, throwTo = (0, 1)}
  ]
...
realInput :: [Monkey]
realInput =
  [ Monkey {items = [52, 78, 79, 63, 51, 94], operation = Mult "old" "13", testDivisibleBy = 5, throwTo = (1, 6)},
    Monkey {items = [77, 94, 70, 83, 53], operation = Add "old" "3", testDivisibleBy = 7, throwTo = (5, 3)},
    Monkey {items = [98, 50, 76], operation = Mult "old" "old", testDivisibleBy = 13, throwTo = (0, 6)},
    Monkey {items = [92, 91, 61, 75, 99, 63, 84, 69], operation = Add "old" "5", testDivisibleBy = 11, throwTo = (5, 7)},
    Monkey {items = [51, 53, 83, 52], operation = Add "old" "7", testDivisibleBy = 3, throwTo = (2, 0)},
    Monkey {items = [76, 76], operation = Add "old" "4", testDivisibleBy = 2, throwTo = (4, 7)},
    Monkey {items = [75, 59, 93, 69, 76, 96, 65], operation = Mult "old" "19", testDivisibleBy = 17, throwTo = (1, 3)},
    Monkey {items = [89], operation = Add "old" "2", testDivisibleBy = 19, throwTo = (2, 4)}
  ]

doRounds :: Int -> World -> World
doRounds n world
  | n > 0 = let w = doRound world in doRounds (n - 1) w
  | otherwise = world

part1 :: Int
part1 =
  let (_, _, inspectCounts) = doRounds 20 (realInput, emptyPendingThrows, emptyInspectCounts)
      top2 = take 2 . reverse . sort . map snd $ Map.toList inspectCounts
   in L.foldl' (\mem i -> mem * i) 1 top2
